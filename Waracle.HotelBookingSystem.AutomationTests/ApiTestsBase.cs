...
namespace Waracle.HotelBookingSystem.AutomationTests
{
    public class ApiTestsBase
    {
        ...
        {
            var response = await RestHttpClient.ExecuteGetAsync<string>($"/api/rooms?checkInDate={checkIn.ToShortDateString()}&checkOutDate={checkOut.ToShortDateString()}&numberOfOccupants=1");
            if (response.StatusCode == System.Net.HttpStatusCode.OK && response.Content != null && response.Content.Count() > 0)
            {
